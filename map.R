...
# Loading datasets
visit_data <- read.csv("visit_data_clean.csv", stringsAsFactors = F, encoding = "UTF-8")
visit_data_coords <- read.csv("visit_data_coords.csv", stringsAsFactors = F, encoding = "UTF-8")

# Loading shpfiles for SG Planning Area polygons
PA <- readOGR("planning-area-2019.kml")
...
map_locations
# saveWidget(map_locations, "map_locations.html")
...
map_heatmap <- merged %>%
  ...
  setView(lat = 1.337896, lng = 103.839627, zoom = 11) #Singapore coordinates
map_heatmap
# saveWidget(map_heatmap, "map_heatmap.html")
