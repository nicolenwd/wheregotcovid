### 1. Script that preprocesses data of public places visited by COVID19 cases
...
# Loading dataset
visit_data_raw <- fread("data/visit_data_raw.csv", header = T, encoding = "UTF-8") %>% select(-V1)
...
visit_data_clean <- visit_data_raw %>%
  ...
  mutate(Store = sub(visit_data_raw[184, "Store"], "Toys'R'Us", Store, fixed = T))

# Replacing values in wrong column
which((visit_data_clean$Location == "McDonald's"), arr.ind = T)
visit_data_clean[82, "Location"] <- "Bedok Mall"
...
write_csv(visit_data_clean, "data/visit_data_clean.csv", col_names = T) 
...
write_csv(visit_data_coords, "data/visit_data_coords.csv", col_names = T)
